...
public final class MetaEntity {
  ...
  public var isValid: Bool {
    guard let (_, _) = getValidated() else { return false }
    return true
  }
  
  /// May be a MetaEntity.Error or a EntitySystem.Error
  public var lastError: Swift.Error? = nil
  ...
  /// Creates a MetaEntity from an entity and checks if it exists within the system. Warning: An entity may exist in two seperate systems
...
//  /// Changes which entity system its registered in. Destroys Entity first in its old system. Throws if registration was unsuccessful.
//  public func register(in entitySystem: EntitySystem, movingComponents: Bool = true) throws {
//    guard let (oldEntity, oldSystem) = getValidated() else { throw lastError ?? throwError() }
//
//    defer {
//      oldSystem.destroy(oldEntity)
//    }
//
//    self.ecs = entitySystem
//    guard let entity = try? entitySystem.newEntity() else {
//      throw Error.couldNotRegisterInSystem
//    }
//    self.entity = entity
//
//    if movingComponents {
//      let components = try getAllComponents()
//      for (_, component) in components {
//        try entitySystem.add(component, to: entity)
//      }
//    }
//  }
